...
export default function Dashboard() {
  ...
  const [periodeGraphique, setPeriodeGraphique] = useState("7");

  const donnéesGraphique = useMemo(() => {
    const nombreJours = Number(periodeGraphique);
    ...
    for (let i = nombreJours - 1; i >= 0; i--) {
      ...
      jours.push({
        ...
        nom: date.toLocaleDateString("fr-FR", {
          weekday: nombreJours === 7 ? "short" : undefined,
          day: nombreJours !== 7 ? "2-digit" : undefined,
          month: nombreJours !== 7 ? "short" : undefined,
        }),
        ventes: 0,
      });
    }
    ...
    (factures || []).forEach((facture) => {
      ...
      if (isNaN(dateFacture.getTime())) return;

      dateFacture.setHours(0, 0, 0, 0);

      const dateString = dateFacture.toISOString().split("T")[0];

      const jour = jours.find((j) => j.date === dateString);

      if (jour) {
        jour.ventes += Number(facture.total || 0);
      }
    });

    return jours;
  }, [factures, periodeGraphique]);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {chargement ? (
          Array(4)
            .fill(0)
            .map((_, i) => <SkeletonCarte key={i} />)
        ) : (
          <>
            <CarteStat
              titre="Ventes Totales"
              valeur={`Ar ${totalVentes.toFixed(2)}`}
              icone={DollarSign}
              tendance="hausse"
              valeurTendance="+12.5%"
              couleur="bg-blue-500"
            ...
            <CarteStat
              ...
              valeur={stockBas}
              icone={AlertTriangle}
              tendance={stockBas > 0 ? "baisse" : "hausse"}
              valeurTendance={stockBas > 0 ? `-${stockBas}` : "0"}
              couleur="bg-orange-500"
            />
            <CarteStat
              titre="Panier Moyen"
              valeur={`Ar ${panierMoyen.toFixed(2)}`}
              icone={ShoppingBag}
              tendance="hausse"
              valeurTendance="+3.2%"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                ...
                <select
                  ...
                  className="border border-slate-200 rounded-lg text-sm text-slate-600 px-3 py-1.5 focus:ring-2 focus:ring-blue-500 outline-none"
                >
        ...
        <div className="lg:col-span-1">
          {chargement ? (
            <SkeletonVentesRecentes />
          ) : (
            <VentesRecentes factures={factures} />
          )}
        </div>
      </div>
    </div>
